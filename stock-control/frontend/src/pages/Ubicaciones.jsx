...
export default function Ubicaciones() {
  ...
  const openEdit   = (u) => { setForm({ name: u.name, type: u.type || '' }); setError(''); setModal(u.id); };
  ...
  const handleDelete = async (id) => {
    ...
    try { await remove(id); notify('Ubicación desactivada'); await load(); }
    catch (e) { setError(e.response?.data?.error || 'Error al desactivar'); }
  };
      ...
      {modal !== null && (
        ...
        >
          ...
          <div className="form-group">
            ...
            <input className="form-control" placeholder="Ej: Depósito A, Heladera 1..."
              value={form.name} onChange={fld('name')} />
          </div>
          <div className="form-group">
            <label className="form-label">Tipo</label>
            <input className="form-control" placeholder="Ej: deposito, heladera, estante..."
              value={form.type} onChange={fld('type')} />
          </div>
        </Modal>
      )}
    </div>
