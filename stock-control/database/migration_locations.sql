-- Migración: soporte multi-ubicación (farmacia, guardia, dispensarios)
-- Ejecutar sobre la base de datos existente stock_control
...
-- Ubicaciones base (Farmacia siempre id=1)
...
-- 3. Columnas de ubicación en movimientos
ALTER TABLE stock_movements
    ADD COLUMN IF NOT EXISTS location_id    INT NULL AFTER product_id,
    ADD COLUMN IF NOT EXISTS to_location_id INT NULL AFTER location_id;

-- Permitir tipo transferencia
ALTER TABLE stock_movements
    MODIFY COLUMN type ENUM('entrada','salida','ajuste','transferencia') NOT NULL;

-- Asignar movimientos históricos a Farmacia
