...
match ($method) {
    'GET'    => listUsers($db),
    'POST'   => createUser($db),
    'PUT'    => $id ? updateUser($db, $id, $payload) : jsonError('ID requerido', 400),
    'DELETE' => $id ? deleteUser($db, $id, $payload) : jsonError('ID requerido', 400),
...
function createUser(PDO $db): void {
    ...
    try {
        ...
        if ($e->getCode() === '23000') {
            jsonError('El username ya existe', 409);
        }
        jsonError('Error al crear usuario: ' . $e->getMessage(), 500);
    }
}

function updateUser(PDO $db, int $id, array $payload): void {
    $data = getBody();
    $role = $data['role'] ?? 'operador';
    if (!in_array($role, ['admin', 'operador'])) jsonError('Rol inválido');
...
function deleteUser(PDO $db, int $id, array $payload): void {
